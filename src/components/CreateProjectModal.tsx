...
export default function CreateProjectModal({ visible, onClose }: Props) {
  ...
  const onSubmit = (data: ProjectFormData) => {
    ...
    createProject(dataWithIconAndColor, {
      onSuccess: () => {
        showSuccess("Succès", "Projet créé avec succès !");
        reset();
        onClose();
      },
      onError: (error: any) => {
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            <View style={styles.header}>
              <View style={[styles.titleIcon, { backgroundColor: (selectedColor || theme.primary) + '15' }]}>
                <Ionicons name="folder" size={24} color={selectedColor || theme.primary} />
              </View>
              <View style={styles.titleContent}>
                <Text style={[styles.title, { color: theme.text }]}>Nouveau Projet</Text>
                <Text style={[styles.subtitle, { color: theme.textSecondary }]}>Créez un nouveau projet</Text>
              </View>
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <View style={[styles.inputContainer, { backgroundColor: theme.background }]}>
                  <Ionicons name="folder-outline" size={20} color={selectedColor || theme.primary} />
                  <TextInput 
              ...
              {PROJECT_ICONS.map((item) => (
                <TouchableOpacity 
                  ...
                  style={[
                    ...
                    selectedIcon === item.icon && { backgroundColor: selectedColor + '20', borderColor: selectedColor }
                  ]}
                >
                  <Ionicons 
                    name={item.icon as any} 
                    size={20} 
                    color={selectedIcon === item.icon ? selectedColor : theme.textSecondary} 
                  />
                </TouchableOpacity>
              ))}
            </View>
