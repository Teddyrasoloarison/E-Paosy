...
export default function DashboardScreen() {
  ...
  return (
    <DashboardShell
      title="Dashboard"
      subtitle={`Bienvenue, ${username ?? ""}`.trim()}
          ...
          >
            <View style={styles.welcomeContent}>
              ...
              <Text style={styles.balanceText}>
                {Math.abs(totalBalance).toLocaleString("fr-FR")} {currency}
              </Text>
              <View style={styles.welcomeStats}>
                <View style={styles.welcomeStatItem}>
                  ...
                  <Text style={styles.welcomeStatText}>
                    {activeWalletsCount} portefeuille(s) actif(s)
                  </Text>
                </View>
                <View style={styles.welcomeStatItem}>
        ...
        <Animated.View style={[getAnimStyle(quickActionsAnim)]}>
          ...
          <View style={styles.quickActions}>
            ...
            >
              ...
              <Text
                style={[styles.actionTextSmall, { color: theme.textSecondary }]}
              >
                Ajouter
              </Text>
            </TouchableOpacity>
        ...
        <Animated.View style={[getAnimStyle(statsAnim)]}>
          <View style={styles.sectionHeader}>
            <View style={styles.sectionTitleRow}>
              <Text style={[styles.sectionTitle, { color: theme.text }]}>
                État de vos portefeuilles
              </Text>
            </View>
            <Text
              style={[styles.sectionSubtitle, { color: theme.textSecondary }]}
            >
              Accéder au montant dans vos portefeuilles
            </Text>
          </View>
          ...
          <View style={styles.statsContainer}>
                ...
                <View style={styles.walletNamesList}>
                  {(showAllActiveWallets
                    ...
                    <View key={wallet.id} style={styles.walletNameItem}>
                      ...
                      />
                      <Text
                        style={[styles.walletNameText, { color: theme.text }]}
                        numberOfLines={1}
                      >
                        {wallet.name}
                      </Text>
                      ...
                      >
                        {(wallet.amount || 0).toLocaleString()} {currency}
                      </Text>
                    </View>
                  ))}
                  {activeWalletsCount > 3 && (
                    <TouchableOpacity
                      onPress={() =>
                        setShowAllActiveWallets(!showAllActiveWallets)
                      }
                    ...
                    >
                      <Text style={[styles.moreText, { color: theme.primary }]}>
                        {showAllActiveWallets
                          ? "Réduire"
                          : `+${activeWalletsCount - 3} voir plus`}
                      </Text>
                    </TouchableOpacity>
                  )}
                </View>
            ...
            <View style={[styles.statCard, { backgroundColor: theme.surface }]}>
              ...
              {inactiveWalletsCount > 0 ? (
                <View style={styles.walletNamesList}>
                  {(showAllInactiveWallets
                    ? inactiveWallets
                    : inactiveWallets.slice(0, 3)
                  ).map((wallet) => (
                    <View key={wallet.id} style={styles.walletNameItem}>
                      <View
                        style={[
                  ...
                  {inactiveWalletsCount > 3 && (
                    <TouchableOpacity
                      onPress={() =>
                        setShowAllInactiveWallets(!showAllInactiveWallets)
                      }
                      style={styles.moreButton}
                    >
                      <Text style={[styles.moreText, { color: theme.primary }]}>
                        {showAllInactiveWallets
                          ? "Réduire"
                          : `+${inactiveWalletsCount - 3} voir plus`}
                      </Text>
                    </TouchableOpacity>
                  )}
                </View>
